...
{ ObjectId.GenerateNewId().ToString(), orderid4, pid1, 10 }
            ...
            migrationBuilder.InsertData(table: "WarehouseStock", columns: new[] { "ID", "ProductID", "AvailableQty" },
                values: new object[,]
                {
   { ObjectId.GenerateNewId().ToString(), pid1, 50 },
    {ObjectId.GenerateNewId().ToString(),  pid2,  150 },
      {ObjectId.GenerateNewId().ToString(), pid3, 175 },
       {ObjectId.GenerateNewId().ToString(), pid4, 250 }
                });
        }
